...
//   const [role, setRole] = useState("DONOR"); // picker if you want

//   const submit = async (e) => {
//     e.preventDefault();
//     await login({ email, password, role });
//     router.replace(role === "DONOR" ? "/donor" : role === "RECEIVER" ? "/receiver" : "/admin");
//   };
...
//             Don’t have an account? <Link href="/register" style={styles.link}>Create Account</Link>
//           </p>
//         </div>
//       </div>
//     </main>
...
export default function LoginPage() {
  ...
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState(""); // unused but kept for UI
  ...
  const submit = async (e) => {
    ...
    else router.replace("/admin"); // ADMIN
  };
